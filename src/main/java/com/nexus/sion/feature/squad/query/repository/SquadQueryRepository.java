...
public class SquadQueryRepository {
    ...
    public List<SquadListResponse> findSquads(SquadListRequest request) {
        ...
        Map<String, List<SquadListResponse.MemberInfo>> memberMap = dsl
                        .select(SQUAD_EMPLOYEE.SQUAD_CODE, MEMBER.EMPLOYEE_NAME,
                                        PROJECT_AND_JOB.JOB_NAME)
                        .from(SQUAD_EMPLOYEE).join(MEMBER)
                        .on(MEMBER.EMPLOYEE_IDENTIFICATION_NUMBER
                                        .eq(SQUAD_EMPLOYEE.EMPLOYEE_IDENTIFICATION_NUMBER))
                        .join(PROJECT_AND_JOB)
                        .on(SQUAD_EMPLOYEE.PROJECT_AND_JOB_ID
                                        .eq(PROJECT_AND_JOB.PROJECT_AND_JOB_ID))
                        .fetchGroups(r -> r.get(SQUAD_EMPLOYEE.SQUAD_CODE),
                                        r -> new SquadListResponse.MemberInfo(
                                                        r.get(MEMBER.EMPLOYEE_NAME),
                                                        r.get(PROJECT_AND_JOB.JOB_NAME)));

        return dsl.selectFrom(SQUAD).where(SQUAD.PROJECT_CODE.eq(projectCode))
                        .orderBy(SQUAD.CREATED_AT.desc()).fetch().stream().map(r -> {
                            ...
                            LocalDate end = start.plusMonths(
                                            r.get(SQUAD.ESTIMATED_DURATION).longValue());
                            String period = start + " ~ " + end;
                            String cost = "₩" + r.get(SQUAD.ESTIMATED_COST).toPlainString();

                            return new SquadListResponse(code, name,
                                            memberMap.getOrDefault(code, List.of()), period, cost);
                        }).toList();
    }
}
